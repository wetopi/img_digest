...
/**
 * resizer gets the image from our gridFS and stores different results in AWS S3
  ...
  obtainImageObject(imgData, function(err, image) {
    ...
    if (err) {
      ...
      resizeToDetail(image, config.img_detail, function(err, image) {
        saveToS3(image, config.img_detail, imgData.filename, function(err) {
          ...
          if (imgData.position == 1) {
            image.clone(function(err, imageListHome) {
              ...
              resizeToList(imageListHome, config.img_list_home, function(err, imageListHome) {
                saveToS3(imageListHome, config.img_list_home, imgData.filename, function(err) {

                  resizeToList(image, config.img_list_store, function (err, image) {
                    saveToS3(image, config.img_list_store, imgData.filename, callback);
                  });
                });
              });
            })
      ...
      deleteImageSource(imgData, function() {
        debug('Deleted image GridFs:' + imgData.filename);
      });
...
/**
 * Get the image from our gridFS and convert it to an image object ready to be
...
function obtainImageObject(imgData, callback) {
  ...
  gridfs.readFile(imgData.filename, function(err, gsData) {

    if (err) {
      err = new VError(err, 'Error obtaining imgData.filename: %s', imgData.filename);
    ...
    lwip.open(gsData.binary, getLwipImgType(gsData.gsObject.contentType), function (err, image) {
      ...
      if (err) {
        err = new VError(err, 'Error when Lwip opens the buffer given by gsData of: %s', imgData.filename);
        console.error(err.message);
      }
 ...
 *
 * @param image
 * @param imgConfig object {dir: string, format: string file extensions}
 * @param imgNameOut file name without extension
 * @param callback required Function(err)
 */
function saveToS3(image, imgConfig, imgNameOut, callback) {

  image.toBuffer(imgConfig.format, {quality: imgConfig.quality},  function (err, buffer) {
    ...
    s3obj.putObject({
      ...
      Key: imgConfig.dir + '/' + imgNameOut + '.' + imgConfig.format,
      Body: buffer
    }, function (err) {

      debug('· Saved to S3 as:' + process.env.AWS_S3_BUCKET + '/' + imgConfig.dir + '/' + imgNameOut + '.' + imgConfig.format);
...
function deleteImageSource(imgData, callback) {
  gridfs.deleteFile(imgData.filename, callback);
}
