...
exports.prepareFs = function() {
    ...
    // Set Input dir
    if(!fs.existsSync(config.in_path)) fs.mkdirSync(config.in_path, DIR_PERMS);

    // Set Output dir
    if(!fs.existsSync(config.out_path))
        fs.mkdir(config.out_path, DIR_PERMS);

    if(!fs.existsSync(config.out_path + config.img_list.dir))
        fs.mkdir(config.out_path + config.img_list.dir, DIR_PERMS);

    if(!fs.existsSync(config.out_path + config.img_detail.dir))
        fs.mkdir(config.out_path + config.img_detail.dir, DIR_PERMS);
    ...
    return amqp.connect(process.env.RABBITMQ_URL).then(function(conn) {
        ...
        return conn.createChannel().then(function(ch) {
            ...
            return ch.assertExchange(config.exchange.name, config.exchange.type, config.exchange.options).then(function() {
                ...
                debug("Published '%s' to exchange with rounting_key: '%s'", message, msgConfig.routing_key  );
                return ch.close();
            });
